...
// staticDepositUtxoSwapCoordinatorFlow is the coordinator side of the fixed-amount static
// deposit claim: it drives the 2PC engine, delegates the nested SSP→user transfer to the
// send-transfer coordinator flow, aggregates the deposit-UTXO spend signature, and builds
// the public InitiateUtxoSwap response.
type staticDepositUtxoSwapCoordinatorFlow struct {
	*StaticDepositUtxoSwapFlowHandler
	...
	response *pbspark.InitiateUtxoSwapResponse
}
...
func (f *staticDepositUtxoSwapCoordinatorFlow) BuildCommitPayload(ctx context.Context, results map[string]*anypb.Any) (proto.Message, error) {
	...
	f.response = &pbspark.InitiateUtxoSwapResponse{
...
// InitiateUtxoSwap is the public 2PC entrypoint for the fixed-amount static deposit claim: an
// external SSP hands the user a Spark transfer in exchange for an on-chain UTXO, and the SE
// co-signs the SSP's spend tx. It runs the coordinator-only validation + idempotency
// short-circuit, fast-fails the user signature before any cross-operator work, collects
// spend-tx FROST round-1 commitments, then drives the engine.
func (o *StaticDepositHandler) InitiateUtxoSwap(ctx context.Context, config *so.Config, req *pbspark.InitiateUtxoSwapRequest) (*pbspark.InitiateUtxoSwapResponse, error) {
	if req.GetRequestType() != pbspark.UtxoSwapRequestType_Fixed {
		return nil, sparkerrors.InvalidArgumentMalformedField(fmt.Errorf("initiate_utxo_swap only supports request_type Fixed, got %s", req.GetRequestType()))
	}
	...
	// The public request carries no confirmation threshold; use the network default.
	threshold := resolveConfirmationThreshold(nil, config, schemaNetwork)
	...
	if err := validateUserSignature(receiverIdentityPubKey, req.GetUserSignature(), req.GetSspSignature(), pbspark.UtxoSwapRequestType_Fixed, schemaNetwork, targetUtxo.Hash().String(), targetUtxo.Vout(), totalAmount, pbspark.HashVariant_HASH_VARIANT_UNSPECIFIED); err != nil {
